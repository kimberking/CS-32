...
{
	if (n2 <= 0)    //special case
		return 1;
	if (n1 <= 1)    //special case
	{
		if (n2 == 1 && a1[0] == a2[0])  
			return 1;
		else
			return 0;   //or else nothing
	}
	if (a1[0] == a2[0])
		return countIncludes(a1 + 1, n1 - 1, a2, n2) + countIncludes(a1 + 1, n1 - 1, a2 + 1, n2 - 1);   //if they match
	else
		return countIncludes(a1 + 1, n1 - 1, a2, n2);   //or else just move the other one, like problem 2	
}
...
void order(string a[], int n)
{
	...
	int firstnot = -1;
	int firstgreat = n;   //make a middle

	separate (a, n, a[n-1], firstnot, firstgreat);  //divide into 2
	order(a + firstnot, n - firstgreat);  //sort right
	separate(a, n, a[n - 1], firstnot, firstgreat);
}